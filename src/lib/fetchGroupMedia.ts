...
function getGroupMediaBaseUrl(): string {
  return (
    import.meta.env.FACEBOOK_GROUP_MEDIA_URL ??
    process.env.FACEBOOK_GROUP_MEDIA_URL ??
    DEFAULT_GROUP_MEDIA_URL
  ).replace(/\/$/, '');
}
...
function getAxiosProxyOptions():
  ...
  | undefined {
  const proxyUrl = import.meta.env.HTTP_PROXY ?? process.env.HTTP_PROXY;
...
function parseMediaFromHtml(html: string, type: 'photo' | 'video', groupSlug: string): ClubMediaItem[] {
  const { jsonData } = findJsonInString(
    html,
    'media',
    (data): data is MediaFeed =>
      typeof data === 'object' &&
      data !== null &&
      !Array.isArray(data) &&
      Array.isArray((data as MediaFeed).edges) &&
      (data as MediaFeed).edges!.length > 0,
  );
